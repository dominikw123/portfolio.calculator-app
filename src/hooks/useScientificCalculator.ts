...
export default function useScientificCalculator() {
    ...
    setCurrentValue(prev => {
      ...
      if (isResult) {
        ...
        if (previousValue && operator) {
          if (isDisplayTooLong(MAX_DISPLAY_LENGTH, previousValue, operator, next)) {
            triggerError(setCurrentValue, setPreviousValue, setOperator);
            return prev;
          }
        }
  ...
  function handleSin() {
    ...
    const radians = parseFloat(currentValue);
    const result = Math.sin(radians).toString();

    safeSetCurrent(() => result, true);
  }
  ...
  function handleCos() {
    ...
    const radians = parseFloat(currentValue);
    const result = Math.cos(radians).toString();
  ...
  function handleTan() {
    ...
    const radians = parseFloat(currentValue);
    const result = Math.tan(radians).toString();
  ...
  function handleEqual() {
    ...
    switch (operator) {
      ...
      case "+":
        ...
        break;
      case "^":
        result = Math.pow(prev, curr).toString();
        break;
    }
